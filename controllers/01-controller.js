...
const addBook = (book) => {
  const mapBookId = book.map(b => b.id)
  const findBook = utils.books.filter(b => mapBookId.some(id => id === b.id))

  if (findBook.length) throw new Error('ya esta el libro en la base de datos.')
